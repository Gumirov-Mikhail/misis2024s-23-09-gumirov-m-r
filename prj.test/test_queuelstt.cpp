...
TEST_CASE_TEMPLATE_DEFINE("QueueLstT", T, queuelstt) {
    ...
    QueueLstT<T> queue_copy_assignment;
    ...
    CHECK_THROWS_WITH((void) queue_copy.Top(), "QueueLstT - try get top from empty queue");
    CHECK_THROWS_WITH((void) queue_copy_assignment.Top(), "QueueLstT - try get top from empty queue");

    QueueLstT<T> queue2;
    queue2 = queue;

    QueueLstT<T> queue_move(std::move(queue));
    CHECK(queue.IsEmpty());
    ...
    QueueLstT<T> queue_move_assignments;
