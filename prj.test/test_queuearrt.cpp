...
TEST_CASE_TEMPLATE_DEFINE("QueueArrT", T, queuearrt) {
    ...
    QueueArrT<T> queue_copy_assignment;
    ...
    CHECK_THROWS_WITH((void) queue_copy.Top(), "QueueArrT - try get top from empty queue");
    CHECK_THROWS_WITH((void) queue_copy_assignment.Top(), "QueueArrT - try get top from empty queue");

    QueueArrT<T> queue2;
    queue2 = queue;

    QueueArrT<T> queue_move(std::move(queue));
    CHECK(queue.IsEmpty());
    ...
    QueueArrT<T> queue_move_assignments;
