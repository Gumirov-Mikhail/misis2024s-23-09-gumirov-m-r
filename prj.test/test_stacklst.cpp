...
TEST_CASE("StackLst - Push, Pop, Top and Clear") {
    ...
    SUBCASE("Top with empty stack") {
        StackLst st1;
        StackLst st2(st1);
        StackLst st3(std::move(st1));
        CHECK(st1.IsEmpty());
        CHECK_THROWS_WITH(void(st1.Top()), "StackLst - try get top from empty stack.");
        CHECK(st2.IsEmpty());
        CHECK_THROWS_WITH(void(st2.Top()), "StackLst - try get top from empty stack.");
        CHECK(st3.IsEmpty());
        CHECK_THROWS_WITH(void(st3.Top()), "StackLst - try get top from empty stack.");
        st1.Push(ch2);
        CHECK_EQ(st1.Top(), ch2);
        st1.Pop();
        CHECK_THROWS_WITH(void(st1.Top()), "StackLst - try get top from empty stack.");
...
TEST_CASE("StackLst - Assignment") {
    ...
    SUBCASE("Assignment with copy empty stack") {
        ...
        CHECK(st4.IsEmpty());
        CHECK_THROWS_WITH(void(st3.Top()), "StackLst - try get top from empty stack.");
        CHECK_THROWS_WITH(void(st4.Top()), "StackLst - try get top from empty stack.");
    }
